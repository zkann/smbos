...
**Always-on option.** For a dashboard that doesn't change URL and survives session ends, reboots, and updates, run `python3 <plugin-root>/scripts/cutover_dashboard.py <sop-dir> install` once. The first install builds a small virtualenv for the app and its web bundle, then registers a macOS LaunchAgent (starts at login) bound to a fixed port with a token persisted 0600, so the URL is stable, bookmark it. `... url` prints it, `... uninstall` removes it. macOS only; elsewhere keep using the per-session server.
