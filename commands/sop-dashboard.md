...
**Always-on option.** For a dashboard that doesn't change URL and survives session ends, reboots, and updates, run `python3 <plugin-root>/scripts/serve_dashboard.py <sop-dir> install` once. It registers a macOS LaunchAgent (starts at login, restarts itself, auto-restarts when the plugin updates) bound to a fixed port with a token persisted 0600, so the URL is stable, bookmark it. `... url` prints it, `... rotate` mints a new one (invalidating the old URL), `... uninstall` removes it. macOS only; elsewhere keep using the per-session server.
