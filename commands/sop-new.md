...
- **Reference, do not paraphrase**: if the process already has canonical pieces (a master procedure doc, a style guide, a checklist, a reusable answer bank) or standing rules captured as project memories (`~/.claude/projects/*/memory/`), POINT to them in Inputs and defer to them in the step that uses them, rather than restating their rules in the SOP. Paraphrasing a living rule-set drops the rules the paraphrase missed and drifts from the source. Sweep for these before saving and wire the relevant ones in; the SOP is a thin orchestrator over the canonical corpus, not a second copy of it. Make the references reachable at runtime: an interactive SOP can read sibling docs and `~/.claude/projects/*/memory/` directly, but a background/prepare-mode SOP runs in the cage (reads only the library + stamped `research_reads`, denies `~/.claude/**`), so add referenced source docs to `research_reads` and either materialize a memory's rule into a readable file or mark the SOP `interactive_only`.
