...
- **Inputs**: what must be known or available before starting. If some of it is PER-RUN information only the owner can supply (which client, which file, amounts), also set frontmatter `run_inputs:` with that list; it gates unattended runs for free instead of letting them spawn, discover the gap, and park at cost.
