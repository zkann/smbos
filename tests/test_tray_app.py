...
def test_tray_plist_shape():
    ...
    assert str(tray.APP) in args
    assert args[args.index("--sop-dir") + 1] == "/tmp/sops"
    assert d["EnvironmentVariables"]["PYTHONPATH"] == "/venv/site-packages"  # deps reach the symlink
...
def test_build_app_bundle_writes_named_plist_and_exec(tmp_path):
    ...
    exe = tray.build_app_bundle(python_exec=fake_python, bundle=bundle, sign=False)  # no codesign in tests
    info = plistlib.loads((bundle / "Contents" / "Info.plist").read_bytes())
    assert info["CFBundleName"] == "SmbOS"          # the name macOS shows, not "Python"
    assert info["CFBundleIdentifier"] == tray.TRAY_LABEL
    assert info["LSUIElement"] is True
    assert exe.is_file()                            # a real copy of the interpreter (not a symlink)
    assert exe.read_text() == "#!/bin/sh\n"         # the interpreter content was copied in
