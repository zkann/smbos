...
def test_lock_reclaim_race_single_winner(tmp_path):
    """N processes racing to reclaim one stale lock: exactly one must win."""
    import subprocess, sys as _sys
    locks = tmp_path / "triggers"
    locks.mkdir()
    (locks / "race.lock").write_text("0 stale\n")
    worker = (
        "import sys; sys.path.insert(0, %r); "
        "from smbos_lib import acquire_run_lock; "
        "print('WON' if acquire_run_lock(%r, 'race') else 'LOST')"
    ) % (str(SCRIPTS), str(tmp_path))
    procs = [subprocess.Popen([_sys.executable, "-c", worker],
                              stdout=subprocess.PIPE, text=True) for _ in range(8)]
    results = [pr.communicate()[0].strip() for pr in procs]
    assert results.count("WON") == 1, results  # winners hold; everyone else sees a live lock
    assert (locks / "race.lock").exists()
