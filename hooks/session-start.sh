...
4. FOLLOW THEIR WAY. The "My way" section of an SOP beats your default approach, even when your default seems better. If you think the SOP is wrong, say so and ask; do not silently override it.
5. DEVIATE TRANSPARENTLY. If a step cannot apply, say so in the moment and remember the deviation for step 6.
6. LEARN. After finishing an SOP-guided task: update frontmatter (last_used, runs; clean_runs +1 if there were no corrections or deviations, else reset clean_runs to 0). If there were deviations or user corrections, propose a specific edit as a before/after diff. Apply only with approval, bump the version, and add a dated changelog line saying what changed and why. If the user declines but the observation matters, offer to record it under "Notes for next revision".
7. PROMOTE. After a draft completes its first real run, set status: active. When an active SOP reaches clean_runs of 3 or more, set status: trusted. Any approved content edit resets clean_runs to 0 and returns a trusted SOP to active. Sub-runs count for the sub-SOP's promotion too.
8. IMPLICIT FEEDBACK. Treat these as SOP feedback even when the user does not phrase them as such: mid-task corrections, re-asking a request in different words, editing your output afterward, telling you to skip a step. Fold them into update proposals (step 6) or trigger-phrase improvements.
