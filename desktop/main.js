...
const { dashboardPort, token, dashboardUrl } = require('./resolve')
...
// GET a token-gated JSON endpoint on the running dashboard. Resolves null on any failure (server
// down, bad JSON) so a transient outage never crashes the poll loop.
function apiGet(pathname) {
  return new Promise((resolve) => {
    const url = `http://127.0.0.1:${dashboardPort()}${pathname}?t=${encodeURIComponent(token())}`
...
function createTray() {
  ...
  tray.setContextMenu(Menu.buildFromTemplate([
    ...
    { label: 'Reload', click: () => { if (win) win.loadURL(dashboardUrl()) } },
    { type: 'separator' },
