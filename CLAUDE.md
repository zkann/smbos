...
- Stdlib-only, zero runtime dependencies, for everything EXCEPT the live-mirror dashboard app (`scripts/dashboard_app.py`, which depends on fastapi/uvicorn per `requirements.txt`). The legacy daemon (`serve_dashboard.py`), the MCP server, `run_sop.py`, the importer, and `smbos_lib.py` stay stdlib-only and must keep running on the system Python 3.9. (The blanket zero-dependency property was deliberately dropped 2026-06-15 for the dashboard rewrite; see the gstack design doc.)
