...
RUNTIME_DIRS = {"pending", "payloads", "triggers", "queue", "work", "active-runs", "active-sessions"}
ARCHIVE_DIR = "archive"
...
# --- Picked-up session liveness ----------------------------------------------
# A picked-up dashboard task opens an interactive Claude session, which (unlike a
# run_sop run) holds no SOP flock. To tell whether that session is still alive, the
# SessionStart hook records the session's process (the hook's $PPID is the durable
# claude process; verified) into active-sessions/<task_id>; the dashboard checks the
# pid here so a closed/crashed session surfaces as stalled instead of a false "live".
ACTIVE_SESSIONS_DIR = "active-sessions"


def _session_marker(sop_dir, task_id):
    return Path(sop_dir) / ACTIVE_SESSIONS_DIR / str(int(task_id))


def _proc_start_sig(pid):
    """An opaque start-time string for `pid` from ps, or None if it has no such row. Used only to
    defend against pid reuse: a recycled pid belongs to a process with a different start time."""
    try:
...
def clear_session(sop_dir, task_id):
...
def session_state(sop_dir, task_id, verify=True):
    ...
    try:
        ...
        return None
    recorded_sig = raw[1] if len(raw) > 1 else ""
    try:
