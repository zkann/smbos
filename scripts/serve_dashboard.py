...
def apply_item(sop_dir, pending_name, index):
    ...
    # absolute path: the launch may open in a project folder or the next SOP's
    # declared folder, not sop_dir, so a basename would be ambiguous/unfindable.
    prompt = (f"Work item #{index + 1} from my parked result file '{p}' through the "
